...
function rtVisible() {
  ...
  return rtRows.filter(r => {
    ...
    return !x.archived && (rtFilter.status==='all' || rtStatus(r)===rtFilter.status)
      ...
      && (!kw || [r.title,x.industry,x.owner,x.deliveryText,...(x.pdfs||[]).map(p=>p.name)].join(' ').toLocaleLowerCase().includes(kw));
  });
}
...
function rtRowHtml(row,index) {
  ...
  return `<tr data-id="${rtEscape(row.id)}">
    ...
    <td><select aria-label="细分行业" data-field="industry" ${busy?'disabled':''}>${rtOptions(rtIndustries(),x.industry,'选择行业')}</select></td>
    ...
    <td>${x.deliveryText?`<div class="rt-result">${rtEscape(x.deliveryText)}</div>`:''}${rtFilesHtml(x.pdfs)}${!done?'<div class="rt-muted">尚未交付</div>':''}<button class="rt-link" data-action="deliver" ${busy||!x.owner?'disabled':''}>${done?'查看 / 更新交付':'＋ 提交交付'}</button></td>
...
function renderResearchTasks() {
  ...
  board.innerHTML=`<div class="rt-wrap">
    ...
    <div class="rt-scroll"><table class="rt-task-table"><colgroup><col style="width:26%"><col style="width:13%"><col style="width:12%"><col style="width:13%"><col style="width:23%"><col style="width:13%"></colgroup><thead><tr><th scope="col">研究问题</th><th scope="col">细分行业</th><th scope="col">承接研究员</th><th scope="col">承接时间</th><th scope="col">交付结果</th><th scope="col">交付日期</th></tr></thead><tbody>${list.map(rtRowHtml).join('')}</tbody></table></div>
  ...
  board.querySelectorAll('tr[data-id]').forEach(tr=>{
    tr.querySelectorAll('[data-field]').forEach(el=>el.onchange=()=>rtChange(tr.dataset.id,el.dataset.field,el.value));
...
function rtOpenQuestion(id) {
  ...
  const dialog=rtDialog(row?'编辑研究问题':'添加研究问题','先记录问题，再由研究员承接。也可以在创建时选择承接人。',`
    ...
    <div class="rt-split"><div><label for="rtIndustry">细分行业</label><select id="rtIndustry" required>${rtOptions(rtIndustries(),x.industry,'请选择细分行业')}<option value="__custom__">＋ 添加细分行业</option></select></div><div><label for="rtOwner">承接研究员</label>${row?`<p style="margin:8px 0">${rtEscape(x.owner||'待承接')}（在清单中选择）</p>`:`<select id="rtOwner">${rtOptions(users.map(u=>u.n),'','暂不指定')}</select>`}</div></div>
    <div id="rtCustomWrap" hidden><label for="rtCustomIndustry">新细分行业名称</label><input type="text" id="rtCustomIndustry" maxlength="60" placeholder="输入行业名称"></div>`);
  document.getElementById('rtIndustry').onchange=e=>{document.getElementById('rtCustomWrap').hidden=e.target.value!=='__custom__';};
  document.getElementById('rtForm').onsubmit=e=>{
    e.preventDefault();
    const title=document.getElementById('rtQuestion').value.trim();
    let industry=document.getElementById('rtIndustry').value;
    if(industry==='__custom__') industry=document.getElementById('rtCustomIndustry').value.trim();
...
function rtOpenDelivery(id) {
  ...
  const dialog=rtDialog('交付研究结果',rtEscape(row.title),`
    ...
    <div class="rt-meta">文字和 PDF 至少提交一项；每个 PDF 不超过 20 MB，最多保留 10 个附件。</div>
  ...
  document.getElementById('rtForm').onsubmit=e=>{
    ...
    if(!deliveryText&&!files.length&&!kept.length){document.getElementById('rtFormError').textContent='请填写文字结果或上传 PDF。';return;}
    if(files.length+kept.length>10){document.getElementById('rtFormError').textContent='最多保留 10 个 PDF 附件。';return;}
    if(deliveryText===(x.deliveryText||'')&&!files.length&&kept.length===(x.pdfs||[]).length){dialog.close();return;}
    rtSubmit(dialog,async()=>{
      ...
      try {
        ...
        await rtPersist(row,{}, {deliveryText,pdfs:[...kept,...uploaded],deliveredAt:new Date().toISOString()});
      } catch(e) {
...
function rtMarkdown(rows=rtRows.filter(r=>!rtExtra(r).archived)) {
  const lines=['# 研究任务','',`导出时间：${rtTime(new Date().toISOString())}（北京时间）`,`任务总数：${rows.length} · 待承接：${rows.filter(r=>rtStatus(r)==='pending').length} · 研究中：${rows.filter(r=>rtStatus(r)==='active').length} · 已交付：${rows.filter(r=>rtStatus(r)==='done').length}`,'','| 研究问题 | 细分行业 | 承接研究员 | 承接时间 | 交付结果 | 交付日期 |','| --- | --- | --- | --- | --- | --- |'];
  rows.forEach(r=>{
    const x=rtExtra(r), links=(x.pdfs||[]).filter(p=>rtSafeUrl(p.url)).map(p=>`[${rtMd(p.name)}](<${rtSafeUrl(p.url).replace(/>/g,'%3E').replace(/</g,'%3C')}>)`);
    lines.push(`| ${rtMd(r.title)} | ${rtMd(x.industry)} | ${rtMd(x.owner||'待承接')} | ${rtTime(x.acceptedAt)} | ${[rtMd(x.deliveryText),...links].filter(Boolean).join('<br>')||'尚未交付'} | ${rtTime(x.deliveredAt)} |`);
  });
  lines.push('','## 任务详情','');
  rows.forEach((r,i)=>{
    const x=rtExtra(r);
    lines.push(`### ${i+1}. ${rtMd(r.title)}`,'',`- 状态：${rtStatusName(r)}`,`- 提出人：${rtMd(r.author)}`,`- 提出时间：${rtTime(r.created_at)}`,`- 细分行业：${rtMd(x.industry)}`,`- 承接研究员：${rtMd(x.owner||'待承接')}`,`- 承接时间：${rtTime(x.acceptedAt)}`,`- 交付日期：${rtTime(x.deliveredAt)}`,'','**交付结果**','',x.deliveryText?rtMd(x.deliveryText):'尚无文字结果','');
